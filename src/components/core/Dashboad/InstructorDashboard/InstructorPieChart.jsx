...
function InstructorPieChart({ instructorData }) {
    ...
    datasets: [
      {
        ...
        data: filterStudentData?.map((course) => course?.totalStudentsEnrolled),
  ...
  const datasetForIncome = {
    ...
    datasets: [
      {
        label: "Total Amount",
        data: filterIncomeData?.map((course) => course?.totalAmountGenerated),
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between gap-4 px-14">
        <PieChartLebels
          data={pieDataFor === "student" ? datasetForStudent : datasetForIncome}
        />
        <Pie
          data={pieDataFor === "student" ? datasetForStudent : datasetForIncome}
          options={option}
