...
function UpdatePassword() {
  ...
  const handlerchangePass = () => {
    ...
    if (value.currentPass === value.newPass) {
      toast.error("Current Password and New Password should have different !!");
    } else {
      ...
      <form onSubmit={handleSubmit(handlerchangePass)}>
        <div className="flex flex-row gap-3 mt-5">
          <div className=" w-full flex flex-col gap-2">
            ...
            />
            <div
              className=" text-richblack-100 text-xl absolute left-[425px] bottom-[40px] cursor-pointer"
              onClick={() => setHide1(!hide1)}
            >
              {hide1 ? <IoEye /> : <IoEyeOff />}
            </div>
            {errors.currentPass && <span>Current Password is required</span>}
          </div>
          <div className=" w-full flex flex-col gap-2">
